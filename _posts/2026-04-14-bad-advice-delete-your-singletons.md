...
That's exactly the shape of code that the **Dependency Inversion Principle** — [the D in SOLID](/2026/05/06/house-rules-d/) — tells you not to write: *depend on abstractions, not concrete classes.* The Singleton Pattern is the purest possible violation of that principle. Every caller in your entire codebase depends on the concrete class by name and reaches it through a globally-resolvable method. There is no abstraction. There is no indirection. The class *is* the contract.
